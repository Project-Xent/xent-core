...
static XentNodeId make_xent_tree(XentContext *ctx, uint32_t node_count) {
    ...
    xent_set_size(ctx, root, 1000.0f, 800.0f);
...
int main(void) {
    const uint32_t sizes[] = {100u, 1000u, 10000u};
    ...
    for (size_t i = 0; i < sizeof(sizes) / sizeof(sizes[0]); ++i) {
        ...
        for (int it = 0; it < iterations; ++it) {
            baseline_layout(&baseline, 0.0f, 0.0f, 1000.0f, 800.0f);
        }
        ...
        for (int it = 0; it < iterations; ++it) {
            xent_layout(ctx, root, 1000.0f, 800.0f);
        }
