...
static void configure_child_behavior(XentCtx *ctx, XentNodeId child, XentProtocol protocol, uint32_t index) {
	...
	if (protocol == XENT_PROTOCOL_SWIFTSTACK) xent_stack_setprio(ctx, child, (index % 3 == 0) ? 1.0f : 0.0f);
}
...
static void run_case(char const *name, XentProtocol protocol, uint32_t nodes) {
	...
	printf(
	  "%s nodes=%u iterations=%d full_total_ms=%.3f full_avg_ms=%.3f cache_skip_avg_ms=%.6f\n", name, nodes, iterations,
	  full_elapsed, full_elapsed / ( double ) iterations, cache_elapsed / ( double ) iterations
	);

	XentProfStats p = xent_profile_get(ctx);
	printf(
