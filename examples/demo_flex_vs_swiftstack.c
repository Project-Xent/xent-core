...
static void build_row(XentContext *ctx,
    ...
    xent_set_size(ctx, icon, 24.0f, 24.0f);
    xent_set_size(ctx, trailing, 20.0f, 20.0f);
...
static void run_case(float width) {
    ...
    xent_set_size(ctx, root, width, 120.0f);
    xent_set_gap(ctx, root, 12.0f);
    xent_set_padding(ctx, root, 4.0f, 4.0f, 4.0f, 4.0f);

    xent_set_flex_grow(ctx, left, 1.0f);
    xent_set_flex_grow(ctx, right, 1.0f);
    xent_set_size(ctx, left, NAN, 48.0f);
    xent_set_size(ctx, right, NAN, 48.0f);
