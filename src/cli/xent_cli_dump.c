...
static void dump_tree_node(XentContext const *ctx, XentNodeId node, FILE *out, uint32_t depth) {
...
void xent_dump_tree(XentContext const *ctx, XentNodeId root, FILE *out) {
	...
	while (top > 0u) {
		...
		dump_tree_node(ctx, frame.node, stream, frame.depth);
		...
		{
			if (!dump_stack_push(&stack, &top, &capacity, (DumpFrame) {child, frame.depth + 1u})) {
				free(stack);
				return;
			}
		}
...
static void dump_semantics_node(XentContext const *ctx, XentNodeId node, FILE *out, uint32_t depth) {
	for (uint32_t i = 0; i < depth; ++i) fputs("  ", out);
...
void xent_dump_semantics_text(XentContext const *ctx, XentNodeId root, FILE *out) {
	...
	uint32_t   top      = 0u;
	uint32_t   capacity = 0u;
	if (!dump_stack_push(&stack, &top, &capacity, (DumpFrame) {target, 0u})) return;

	while (top > 0u) {
		DumpFrame frame = stack [--top];
		dump_semantics_node(ctx, frame.node, stream, frame.depth);
		for (XentNodeId child = ctx->nodes.topology.last_child [frame.node]; child != XENT_NODE_INVALID;
		  child               = ctx->nodes.topology.prev_sibling [child])
		{
			if (!dump_stack_push(&stack, &top, &capacity, (DumpFrame) {child, frame.depth + 1u})) {
