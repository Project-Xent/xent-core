...
static const char *xent_protocol_name(XentProtocol p) {
    switch (p) {
...
static const char *xent_semantic_name(XentSemanticRole role) {
    switch (role) {
        case XENT_SEMANTIC_ROOT:
            return "root";
        case XENT_SEMANTIC_CONTAINER:
            return "container";
        case XENT_SEMANTIC_TEXT:
            return "text";
        case XENT_SEMANTIC_BUTTON:
            return "button";
        case XENT_SEMANTIC_IMAGE:
            return "image";
        case XENT_SEMANTIC_CUSTOM:
            return "custom";
...
static void dump_tree_rec(const XentContext *ctx, XentNodeId node, FILE *out, uint32_t depth) {
    ...
    fprintf(out,
            "[%u] rect=(%.1f,%.1f %.1fx%.1f) protocol=%s role=%s",
            node,
            ctx->nodes.abs_x[node],
            ctx->nodes.abs_y[node],
            ctx->nodes.decided_w[node],
            ctx->nodes.decided_h[node],
            xent_protocol_name((XentProtocol)ctx->nodes.protocol[node]),
            xent_semantic_name((XentSemanticRole)ctx->nodes.semantic_role[node]));
...
static void dump_semantics_rec(const XentContext *ctx, XentNodeId node, FILE *out, uint32_t depth) {
    for (uint32_t i = 0; i < depth; ++i) {
        fputs("  ", out);
    }
...
void xent_dump_semantics_text(const XentContext *ctx, XentNodeId root, FILE *out) {
    ...
    if (target == XENT_NODE_INVALID) {
        target = ctx->last_layout_root;
    }
    if (!xent_is_valid_node(ctx, target)) {
        return;
    }
