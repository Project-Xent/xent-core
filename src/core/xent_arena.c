...
#define GROW_ARRAY(field)                                                                                     \
	...
	GROW_ARRAY(layout.style_w_percent);
	GROW_ARRAY(layout.style_h_percent);
	GROW_ARRAY(layout.aspect_ratio);
...
#undef GROW_ARRAY
	...
	for (uint32_t i = old_cap; i < new_cap; ++i) {
		...
		n->layout.style_w_percent [i]           = NAN;
		n->layout.style_h_percent [i]           = NAN;
		n->layout.aspect_ratio [i]              = NAN;
