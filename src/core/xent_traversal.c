...
static void xent_init_visit_parent(
  XentTraversalVisit *visit, XentNodeId node, XentRect layout, XentTraversalVisit const *parent
) {
	visit->node        = node;
...
static void xent_apply_traversal_effects(XentTraversalVisit *visit, XentTraversalOptions const *options) {
	...
	if (visit->effects.clips_children) visit->effective_clip = xent_rect_intersection(visit->effective_clip, visit->screen_rect);
}
...
static void xent_leave_traversal_frame(TraversalFrame *frame, XentTraversalOptions const *options, bool *stopped) {
	if (!options->leave || frame->skip_children) return;
