...
static void ime_stub_on_register(XentContext *ctx, const XentPlugin *plugin) {
    (void)ctx;
...
static void ime_stub_on_semantic_action(XentContext *ctx, XentNodeId node, const char *action, const XentPlugin *plugin) {
    (void)ctx;
    FILE *out = plugin && plugin->user_data ? (FILE *)plugin->user_data : stdout;
