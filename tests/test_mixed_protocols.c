...
int main(void) {
	...
	xent_set_size(ctx, root, (XentSize) {400.0f, 100.0f});
	xent_set_gap(ctx, root, 4.0f);

	xent_set_protocol(ctx, left, XENT_PROTOCOL_ABSOLUTE);
	xent_set_size(ctx, left, (XentSize) {120.0f, 100.0f});
	...
	xent_set_size(ctx, right, (XentSize) {NAN, 100.0f});

	xent_set_is_spacer(ctx, right_spacer, true);
	xent_set_size(ctx, right_tail, (XentSize) {30.0f, 30.0f});
