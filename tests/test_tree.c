...
static void     test_lifecycle(
  ...
  void *userdata
	...
	( void ) new_parent;
	( void ) userdata;
	if (event == XENT_NODE_EVENT_DESTROY) lifecycle_destroy_count++;
}
...
int main(void) {
	...
	TEST_ASSERT(xent_get_first_child(ctx, root) == a);
	TEST_ASSERT(xent_get_last_child(ctx, root) == b);
	TEST_ASSERT(xent_get_next_sibling(ctx, a) == b);
	TEST_ASSERT(xent_get_prev_sibling(ctx, b) == a);
	...
	TEST_ASSERT(xent_get_prev_sibling(ctx, b) == XENT_NODE_INVALID);
	TEST_ASSERT(xent_get_next_sibling(ctx, a) == c);
	TEST_ASSERT(xent_get_prev_sibling(ctx, c) == a);
	TEST_ASSERT(xent_get_last_child(ctx, root) == c);
	TEST_ASSERT(xent_get_child_count(ctx, root) == 2u);
